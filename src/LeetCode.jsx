...
function LeetCode() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://kush-sahu.github.io/Final-sheet2.json/leetcode.json');
    ...
    <h2 id='heading'>450 Top DSA Questions to Crack any Technical Round</h2>
      {Object.entries(groupedCards).map(([topic, cards]) => (
        <div key={topic} className="topic-container-App1">
          <h2 className="topic-heading-App1" onClick={() => toggleTopic(topic)} style={{ backgroundColor: 'rgb(254, 134, 11)' }} >
            {topic}
          </h2>
          ...
          {expandedTopics[topic] && (
            <div className='insideTopic-App1'>
             ...
             <   div key={index} className="card flex" style={{ backgroundColor: 'rgb(254, 218, 1)',flexDirection:'row' }}>
                  <div>
                    <button className='gfgimage1' onClick={() => handleChoose(topic, index)} style={{ color: selectedCards[topic] && selectedCards[topic].includes(index) ? 'white' : 'brown', backgroundColor:selectedCards[topic] && selectedCards[topic].includes(index) ? 'rgb(254, 134, 11)':'white' }}>
